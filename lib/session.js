...
exports.create = function(debuggerPort, config) {
  var sessionInstance,
      debug = createDisconnectedDebuggerClient('node-inspector server was stopped'),
      conn = null,
  ...
  function createDisconnectedDebuggerClient(reason) {
    return {
      request: function() {
      ...
      sendDebugRequest: function(command, args, callback) {
        this.sendDetachedEventToFrontEnd();
        callback('debugger not connected');
      },

      sendDetachedEventToFrontEnd: function() {
    ...
    attach: {
      value: function(done)
      {
        ...
        debug.on('close', function() {
          ...
          var reasonStringForFrontEnd = closeReason.replace(/\n/g, '. ');
          debug = createDisconnectedDebuggerClient(reasonStringForFrontEnd);
          debug.sendDetachedEventToFrontEnd();
